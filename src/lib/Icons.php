...
/**
 * Catálogo de íconos SVG inline.
 *
 * Estilo: Lucide / Heroicons outline — trazo de 1.8-2px, esquinas redondas,
 * pensados para verse bien entre 16px y 32px.
 *
 * Uso: `<?= Icons::svg('check', 'w-5 h-5 text-brand-600') ?>`
 *
 * Cuando se necesite el mismo ícono con el atributo `fill` en vez de `stroke`
 * (ej. WhatsApp logo), se define explícitamente con su markup completo.
 */
class Icons {
    // Mapa nombre → markup interior del SVG (sin la etiqueta <svg> exterior,
    // a menos que requiera `fill` propio).
    private const PATHS = [
        // ─── Status / feedback
        ...
        // ─── Categorías
        'pill'         => '<rect x="2" y="9" width="20" height="6" rx="3"/><path d="M12 9v6"/>',           // vitaminas
        ...
        'zap'          => '<polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/>',                   // energia
        'leaf'         => '<path d="M5 21c1-.5 8-3 11-7s5-9 5-12c0 0-7 .5-13 3S2 12 5 21z"/><path d="M5 21c0-7 5-13 12-13"/>', // digestivos
        'stethoscope'  => '<path d="M4 4v8a6 6 0 0012 0V4"/><circle cx="18" cy="18" r="3"/><path d="M10 14v4a4 4 0 008 0"/>',  // veterinarios
        'muscle'       => '<path d="M14 11a5 5 0 100 10 5 5 0 000-10zM6 7a3 3 0 014 0M5 18l-1 4M8 21l1-4"/>', // bodybuilder
        'shield'       => '<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>',                       // protectores
        'horseshoe'    => '<path d="M6 4v7a6 6 0 0012 0V4M5 13h2M17 13h2M5 17h2M17 17h2M5 21h2M17 21h2"/>', // cascos / default
        'star'         => '<polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>', // marca propia
        'recycle'      => '<polyline points="14 16 18 20 22 16"/><polyline points="2 12 6 8 10 12"/><path d="M18 20V10a5 5 0 00-9-3M6 8v10a5 5 0 009 3"/>', // recuperacion
    ...
    /**
     * Devuelve el `<svg>` listo para inyectar en HTML.
     *
     * @param string $name   nombre del ícono en PATHS
     * @param string $cls    clases CSS adicionales (ej. "w-5 h-5 text-brand-600")
     * @param array  $opts   ['size' => null, 'stroke' => '2', 'fill' => 'none']
     */
    public static function svg(string $name, string $cls = 'w-5 h-5', array $opts = []): string {
        ...
        // El WhatsApp icon ya tiene su propio fill/stroke en el markup
        if ($name === 'whatsapp') {
...
// Function helper para conveniencia en templates
